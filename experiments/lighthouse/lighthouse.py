...
class lighthouse():
    ...
    def lighthouse_state(self, **kwargs):
        config = self.config
        ...
        if self.state is None:
            self.state = {
                ...
                "pins": [],
                "quota": float(self.config.baseConfig["LIGHTHOUSE"]["quota"]),
                "usage": "0",
                "remaining": float(self.config.baseConfig["LIGHTHOUSE"]["quota"]) - 0  
            }
        else:
            self.state = self.lighthouse_calc_state(self.state)
            
        state = self.state
        return state
    
    def lighthouse_calc_state(state):
